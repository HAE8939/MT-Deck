...
`image` 用于把一张**参考图 / 效果图**与提示词关联起来——填写后，详情视图会把这张图和 prompt 正文并排展示，方便你对照图去回忆、复用当初那条提示词。图片统一放在提示词库根目录的 `src/` 文件夹中，值填相对于提示词库的路径，例如 `example.png` 或 `src/example.png`（详情视图默认在裸文件名前补上 `src/`）。注意 `image:` 须用半角冒号，全角 `：` 不会被解析。
